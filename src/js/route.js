...
class Router extends String {
    ...
    withQuery(params) {
        this.queryParams = Object.assign(Object.assign({}, this.queryParams), params);
        return this;
    }
    hydrateUrl() {
        if (this.hydrated)
            return this.hydrated;
        let hydrated = this.template.replace(/{([^}]+)}/gi, (tag, i) => {
            let keyName = this.trimParam(tag), defaultParameter, tagValue;
    ...
    extractParams(uri, template, delimiter) {
        ...
        return templateParts.reduce((params, param, i) => param.indexOf('{') === 0 &&
            param.indexOf('}') !== -1 &&
            uriParts[i]
    ...
    get params() {
        ...
        return Object.assign(Object.assign({}, this.extractParams(window.location.hostname, namedRoute.domain || '', '.')), this.extractParams(window.location.pathname.slice(1), namedRoute.uri, '/'));
    }
    parse() {
